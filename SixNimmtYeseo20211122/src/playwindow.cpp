...
void PlayWindow::computerTurn(){
            ...
            {
                if(computer_current<boardStack1.top()&&computer_current<boardStack2.top()&&computer_current<boardStack3.top()&&computer_current<boardStack4.top())
                {
                   ui->playertotalLabel_2->setText("total: " + QString::number(a+1));
                 }
                    //열을 선택하라는 문구 띄우고,선택했으면 그 카드들 없애고 벌점맥이고 computer_current놓기//선택 알고리즘 필요
                else
                {
                    ...
                    if(difi==1)
                    {
                        ...
                        {
                            ui->card2Label_7->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack1.size()==3)

                        {
                            ui->card3Label_7->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack1.size()==4)

                        {
                            ui->card4Label_7->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack1.size()==5)//먹어야 됨

                        {
                            ui->card1Label_12->setPixmap(cardHolder.at(computer_current));
                        }
                    }
                    ...
                    else if(difi==2)
                    {
                        ...
                        {
                                ui->tableLabel_2->setPixmap(cardHolder.at(computer_current));//player_current가 a+1한거니까
                        }
                        else if(boardStack2.size()==2)

                        {
                            ui->card2Label_4->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack2.size()==3)

                        {
                            ui->card3Label_4->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack2.size()==4)

                        {
                            ui->card4Label_4->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack2.size()==5)//먹어야 됨

                        {
                            ui->card5Label_4->setPixmap(cardHolder.at(computer_current));
                        }
                    }
                    else if(difi==3)
                    {
                        ...
                        {
                                ui->tableLabel_3->setPixmap(cardHolder.at(computer_current));//player_current가 a+1한거니까
                        }
                        else if(boardStack3.size()==2)

                        {
                            ui->card2Label_5->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack3.size()==3)

                        {
                            ui->card3Label_5->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack3.size()==4)

                        {
                            ui->card4Label_5->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack3.size()==5)//먹어야 됨

                        {
                            ui->card1Label_13->setPixmap(cardHolder.at(computer_current));
                        }
                    }
                    else
                    {
                        ...
                        {
                                ui->tableLabel_4->setPixmap(cardHolder.at(computer_current));//player_current가 a+1한거니까
                        }
                        else if(boardStack4.size()==2)

                        {
                            ui->card2Label_6->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack4.size()==3)

                        {
                            ui->card3Label_6->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack4.size()==4)

                        {
                            ui->card4Label_6->setPixmap(cardHolder.at(computer_current));
                        }
                        else if(boardStack4.size()==5)//먹어야 됨

                        {
                            ui->card1Label_11->setPixmap(cardHolder.at(computer_current));
                        }
                    }
                    }
